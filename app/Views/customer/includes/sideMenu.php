<aside class="col-md-3">
		<nav class="list-group">
			<a class="list-group-item active" href="<?=base_url('customer/dashboard')?>"> Account overview  </a>
			<!-- <a class="list-group-item" href="page-profile-address.html"> My Address </a> -->
			<a class="list-group-item" href="<?=base_url('customer/orders')?>"> My Orders </a>
			<!-- <a class="list-group-item" href="page-profile-wishlist.html"> My wishlist </a>
			<a class="list-group-item" href="page-profile-seller.html"> My Selling Items </a> -->
			<a class="list-group-item" href="<?=base_url('customer/setting')?>"> Settings </a>
			<a class="list-group-item" href="<?= base_url('logout'); ?>"> Log out </a>
		</nav>
	</aside> <!-- col.// -->